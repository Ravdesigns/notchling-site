...
[ "$(curl -s -m 20 "$H/version.txt" | tr -d '[:space:]')" = "1.10.1" ] && ok "version.txt 1.10.1" || no "version.txt is $(curl -s -m 20 "$H/version.txt" | tr -d '[:space:]')"
...
if unzip -q "$T/n.zip" -d "$T" 2>/dev/null; then
  ...
  [ "$VER" = "1.10.1" ] && ok "bundle is v1.10.1" || no "bundle is v$VER"
